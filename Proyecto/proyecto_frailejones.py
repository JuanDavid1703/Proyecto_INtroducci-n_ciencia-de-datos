...
# Clasificación de especies de frailejón en el sector del páramo del Sumapaz a través de la morfología foliar  
...
<img title="Páramo" src="https://github.com/JuanDavid1703/Proyecto_INtroducci-n_ciencia-de-datos/raw/main/Proyecto/Imagenes/Paramo.jpg" width="500">
...
<img title="Pisos térmicos" src="https://github.com/JuanDavid1703/Proyecto_INtroducci-n_ciencia-de-datos/raw/main/Proyecto/Imagenes/Pisostermicos.jpg" width="350">
...
<img src="https://github.com/JuanDavid1703/Proyecto_INtroducci-n_ciencia-de-datos/raw/main/Proyecto/Imagenes/1200px-Leaf_morphology_es.png" width="350">
...
<img src="https://github.com/JuanDavid1703/Proyecto_INtroducci-n_ciencia-de-datos/raw/main/Proyecto/Imagenes/Bandeja.png" width="350">
...
Los datos consisten en 180 imágenes de hojas de frailejones de diferentes tamaños en 4 diferentes capas de colores, estas fueron obtenidas mediante fotografías tomadas en el páramo del Sumapaz (Jimenez 2020), cada fotografía tenía en promedio de 3 a 4 hojas de frailejón.

En este estudio se realizará un análisis de la morfología de las hojas en diferentes medidas y tamaños. A cada imagen se le realizará la separación de la hoja con le fondo, algo necesario para poder aplicar el algortimo deseado en la estracción de características de contorno y características básicas, una pequeña muestra se puede observar en la imagen

<img src="https://github.com/JuanDavid1703/Proyecto_INtroducci-n_ciencia-de-datos/raw/main/Proyecto/Imagenes/primicia.jpg" width="500">

**Riesgos:**

* Dado que se necesitó quitar el fondo de la imagen, algunas imágenes no quedaron con contornos suaves, esto quizas genere un error al momento de la extracción de contornos, además los tamaños de las imágenes fueron alterados al momento de separarlas del fondo dado que podría haber habido varias hojas en una misma fotografía.

* Es posible que los datos no sean suficientes o no tengan la calidad y la cantidad de características suficientes para realizar la extracción de características de contorno y poder clasificar cada imagen, como recurso opcional se tendrá en cuenta el uso de redes convolucionales como herramienta extra para la extracción de características de imágenes.
...
[4] ***Leonel Olivares, Jorge Victorino, Francisco Gómez***, "*Automatic Leaf Shape Category Discovery*", Universidad Central, 2016.

#IDA

##Resumen de configuración de los datos:
Los datos consisten en 180 imágenes de hojas de frailejones de diferentes tamaños en 4 diferentes capas de colores, estas fueron obtenidas mediante fotografías tomadas en le páramo del Sumapaz, cada fotografía tenía en promedio de 3 a 4 hojas de frailejón.  

De estos datos se desea realizar una apropiada clasificación de especies de frailejones mediante la caracterización y clasificación de hojas de frailejón usando morfología foliar. 

Como primicia se quizo reconocer los tipos de datos que se tenían, para ello se debió separar las hojas del fondo de cada fografía, en este proceso se usó "Adobe photshop express", programa que fue escogido después de intentar con varios, una de las más fuertes complicaciones en la separación de las hojas del fondo se debía a la calidad de la iluminación de cada imagen, como no se tenía buena iluminación, no se pudieron separar todas las hojas de todas las imágenes, se lograron obtener 314 hojas de frailejón separadas y en su mayoría bien contrastadas con su fondo, la proporción que hay de cada especie morfológicamente escogida se prsenta en siguiente diagrama.
...
Después de esto, la intención es obtener características que permitan distinguir cada hoja, las primeras características que se obtienen están relacionadas con la geometría básica de una elipse, dado que son figuras cerradas es posible contrastar cada hoja con una posible elipse con características distinguibles entre ellas, las segundas características a obtener son los contornos de las hojas ya que esto permitirí cada hoja usando la tranformada discreta de Fourier, dado que los contronos son curvas en el plano, es posible caracterizar cada hoja por sus armónicos, hubo algunos inconvenientes dada la separación realizada del fondo en cada hoja ya que algo de ruido se mezcló en algunas imágenes, por esto se tuvo que proceder en realizar una limpieza exhaustiva y además arreglar las tonalidades del color de cada imagen.


Depués de haber obtenido con éxito los contornos se procedió a hallar características básicas elípticas de cada hoja, las características escojidas fueron:

* El área
* El perímetro
* Las medidas de los ejes axiales, el mayor y el menor.
* La eccentricidad

A partir de las anteriores características se obtuvieron los siguientes descriptores:
...
Las anteiores características fueron escogidas dado que cada hoja se puede encerrar en una elipse que está caracterizada por ciertos parámetros, esto nos permitiría caracterizar mejor cada hoja de frailejón, además como otra medida, al usar los contornos se pueden identificar las curvas que representan cada componenete del conjunto de puntos como se ve en la imagen siguiente: 

<img title="Páramo" src="https://github.com/JuanDavid1703/Proyecto_INtroducci-n_ciencia-de-datos/raw/main/Proyecto/Imagenes/contorno.png" width="1000">

La descripción general d los datos básicos de la muestra se puede observar a través de las características obtenidas mencionados anteriormente, los datos obtenidos se presentan en la siguiente tabla, como se puede ver son valores entre 0 y uno, además como son razónes es posible quitar las unidades de medida de cada imágen teniendo solamente proporciones para distinguir las hojas:


<img title="Páramo" src="https://github.com/JuanDavid1703/Proyecto_INtroducci-n_ciencia-de-datos/raw/main/Proyecto/Imagenes/Tabla_inicial.png" width="600">


La matriz de correlación entre cada par de datos se puede observar en la siugiente matriz de correlación, esto puede ser debido a la variación de las dimensiones de las imágenes analizadas

<img title="Páramo" src="https://github.com/JuanDavid1703/Proyecto_INtroducci-n_ciencia-de-datos/raw/main/Proyecto/Imagenes/Distri_basicas.png" width="1300" height="800">


Armonicos de Fourier:

Dada una función, la transfomada de Fourier permite describir esa función en terminos de una serie de funciones trigonométricas en donde su representación esta dada por el espacio de espectro, este espacio lo conforman los armónicos.

En el análisis de imágenes se usa la transfomada rápida de Fourier, un mecanismo que permite analizar y caracterizar el contorno o los bordes de una imagen binarizada

$$X_k=\sum_{n=0}^{N-1}{x_n e^{\frac{-2\pi i}{N}kn}}$$

con $k=1,2,3 \ldots N-1$

Los armónicos permiten identificar puntas y bordes suaves, cuando los armónicos son de valores de alta variación, eso podría representar puntas o esquinas en las imágenes pero si son de baja variabilidad podria representar bordes suaves con curvas suaves.

en la siguiente imagen se puede apreciar 4 diferentes representaciones de los armónicos de los bordes 4 distintas hojas tomadas al azar y se puede observar que en dos de las cuatro imágenes hay mayor variación entre los valores que las otras dos:


<img title="Páramo" src="https://github.com/JuanDavid1703/Proyecto_INtroducci-n_ciencia-de-datos/raw/main/Proyecto/Imagenes/Armonicos.png" width="900" height="600">


Para poder escoger unas adecuadas caracterpistica de deberían tomar los armónicos de mayor variación, se debe tener en cuenta la dimensionalidad de las características por ende se deben tomar menos armónicos para el espacio de características.


**Conclusiones**:

Dada la estracción de las características mostradas anteriormente se puede crear el espacio de características el cual servirá para realizar la respectivo tratamiento de los datos usando diferentes métodos de clasificación la cual es la finalidad del proyecto.
"""